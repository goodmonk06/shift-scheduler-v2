...
6. **AIシフト自動生成機能** ✨NEW (2025-11-15)
   - **バックエンド**: `server/aiShiftGenerator.ts` (709行) - 完全実装済み
   - **tRPC API**: `shifts.generateAI` エンドポイント実装済み
   - **フロントエンド**: `client/src/components/ShiftEditor.tsx` にAI生成ボタン実装済み
   - **実装ガイド**: `client/src/config/AI_GENERATION_GUIDE.md` (795行の詳細ドキュメント)

   **主な機能:**
   - 2段階生成方式: パートタイム職員 → 正職員の順で最適化
   - 制約緩和ロジック: 失敗時に段階的に制約を緩和 (レベル0-3)
   - 希望休完全対応: 全職員の希望休を自動反映
   - ルール遵守: 職場ルール・職員ルール完全準拠
   - 検証機能: shiftValidator モジュールでハード制約チェック
   - トランザクション管理: DB保存の原子性保証

   **使用方法:**
   1. 管理者画面でステータス「draft」のシフトを作成
   2. 職員が希望休を申請
   3. シフト編集画面で「AI生成」ボタンをクリック
   4. プロンプトプレビューダイアログで確認
   5. 実行すると新しい「ai_generated」ステータスのシフトが作成される

   **技術詳細:**
   - OpenAI API使用 (JSON Schema Structured Outputs)
   - invokeLLM() ラッパー関数でOpenAI/Anthropic両対応
   - リトライロジック付き (最大3回、段階的制約緩和)
   - 詳細ログ出力でデバッグ可能
...
### 1. API無限ループ修正（2025年11月15日）⚠️ 最優先

**問題の概要**
`workTimeSlots.list` APIが無限に呼び出され、サーバーとデータベースに過負荷がかかっていた重大な問題。

**原因**
`ShiftTableView.tsx`（113行目）のuseEffect依存配列に `[toast]` を含めていたため、toastオブジェクトの参照が変わるたびにuseEffectが再実行され、無限ループが発生。
...
**影響範囲**
- シフト編集画面（管理者のみ）
- サーバーリソースとデータベースに過大な負荷
- Railway/Aivenのコスト増加リスク
...
### 2. useEffect インポートエラー修正（2025年11月15日）
...
### v1.1.0 (2025年11月15日) - 現在のバージョン

**新機能:**
- ✨ 施設イベント管理機能（管理者が登録、全職員に自動反映）
- 📅 祝日データ対応（2025年全祝日）
- ♻️ 施設イベント自動更新（ページ可視化時・5分間隔）

**重要な修正:**
- 🐛 API無限ループ修正（workTimeSlots.list）
- 🐛 useEffectインポートエラー修正（EmployeeHome）
...
### useEffect依存配列の注意点
- **toast、navigate等のフック返り値を依存配列に含めない**
- これらは毎回新しいオブジェクト参照になる可能性があり、無限ループを引き起こす
- データフェッチは空配列`[]`で初回マウント時のみ実行
- 必要な場合のみ、プリミティブ値（ID、年月等）を依存配列に含める
