...
### 問題の概要
ホーム画面に追加したPWA（スタンドアローンモード）で、タスクキル後にアプリを再開すると白画面（上下に紫帯のみ）になる問題。Safari通常モードでは発生しない。

### 原因
1. **Service Workerのキャッシュ戦略**: API認証リクエストまでキャッシュしていたため、古い認証失敗レスポンスが返される
2. **認証チェックの失敗**: Cookie/セッションが失われた状態で認証チェックが失敗し、ユーザー情報が取得できない
3. **エラーハンドリング不足**: 認証失敗時のログが不十分で原因特定が困難

### 修正内容

**1. Service Worker (client/public/sw.js)**
- キャッシュバージョンをv1→v2に更新（既存PWAの強制更新）
- API呼び出し (`/api/*`, `/trpc/*`) は**ネットワーク優先**に変更
- 静的アセットのみキャッシュファースト戦略を適用

**2. App.tsx**
- アプリ可視化時に認証状態を再チェック（visibilitychange イベント）
- 認証チェックの詳細ログを追加
- PWAモードでも確実に認証状態を復元

**3. authService.ts**
- getCurrentUser()に詳細なログを追加
- 認証リクエストの成功/失敗を確認可能に
...
### テスト手順
1. 本番環境にデプロイ後、ブラウザでログイン
2. 「ホーム画面に追加」でPWAインストール
3. PWAを開いてログイン
4. アプリをタスクキル
5. ホーム画面のアイコンから再起動
6. コンソールログを確認（認証チェックの詳細が表示される）
