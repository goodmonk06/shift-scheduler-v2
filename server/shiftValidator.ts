...
export function validateShift(context: ValidationContext): ValidationResult {
  ...
  errors.push(...validateRequiredStaffing(context));
 ...
 * 3. 必要人数が満たされているかチェック
 */
function validateRequiredStaffing(context: ValidationContext): ValidationError[] {
  const errors: ValidationError[] = [];
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      // 必要人数に対して80%未満の場合はエラー
      const coverageRatio = actualStaffing / required.requiredCount;
      if (coverageRatio < 0.8) {
        errors.push({
          type: "hard_constraint",
          category: "understaffed",
          message: `必要人数を大幅に下回っています（必要: ${required.requiredCount}人、実際: ${actualStaffing.toFixed(1)}人）`,
          date: dateStr,
          details: {
            hour,
