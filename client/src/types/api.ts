...
export interface EmployeeConstraints {
  // 基本的な勤務可能時間（曜日ごと）
  workableTimeSlots: {
    dayOfWeek: 0 | 1 | 2 | 3 | 4 | 5 | 6;  // 0=日, 1=月, ..., 6=土
    startTime: string;  // HH:MM形式
    endTime: string;  // HH:MM形式
  }[];
  
  // 勤務不可の曜日パターン
  unavailableDayPatterns: {
    type: "fixed" | "nth-week";  // fixed: 毎週固定, nth-week: 第N週
    dayOfWeek?: 0 | 1 | 2 | 3 | 4 | 5 | 6;  // typeがfixedの場合
    nthWeek?: number;  // typeがnth-weekの場合 (1=第1週, 2=第2週, ...)
    description: string;  // AIへの説明用 例: "第2・第4土曜日は不可"
  }[];
  ...
  // 追加の制約（自然言語でAIに伝える）
  additionalConstraints: string;  // 例: "子供の送迎のため、水曜日は17時まで"
}
...
export interface Shift {
  ...
  // バージョン管理用（別IDで保存する場合）
  baseShiftId?: number | null;  // 元のシフトID（改訂版の場合）
...
export type ShiftStatus = 
  | "vacation_only"      // 1. 希望休のみ挿入
  | "ai_generated"       // 2. AI生成後
  | "tentative"          // 3. 仮確定（人間が手を加えた）
  | "tentative_revised"  // 4. 仮確定（改）- 追加希望受付後
  | "final"              // 5. 最終シフト
  | "actual";            // 6. 日々変更後の実際の勤務（勤怠管理連携用）
...
export interface LeaveRequest {
  ...
  startTime?: string;  // HH:MM形式（時間指定の場合のみ）
  endTime?: string;  // HH:MM形式（時間指定の場合のみ）
