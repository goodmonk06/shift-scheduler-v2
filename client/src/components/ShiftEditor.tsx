...
export function ShiftEditor({ shiftId, onBack }: ShiftEditorProps = {}) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log('[ShiftEditor] Raw employees data:', employeesData.slice(0, 3));
      ...
      console.log('[ShiftEditor] Formatted employees:', formattedEmployees.slice(0, 3));
      ...
      console.log('[ShiftEditor] Converting shiftDetails to assignments. First detail:', shiftDetails[0]);
      const formattedAssignments: ShiftAssignment[] = shiftDetails.map((detail, index) => {
        const employee = employeesData.find(emp => emp.id === detail.employeeId);
        if (index < 3) {
          console.log(`[ShiftEditor] Detail[${index}]:`, {
            detailEmployeeId: detail.employeeId,
            foundEmployee: employee ? { id: employee.id, name: employee.name } : null
          });
        }
      ...
      // デバッグ用：最初の5件のassignmentsを詳細表示
      console.log('[ShiftEditor] First 5 assignments detail:');
      allAssignments.slice(0, 5).forEach((a, i) => {
        console.log(`  [${i}] ${a.date} - ${a.employeeName}: ${a.timeSlotName || 'N/A'}`);
      });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2">
                ...
                {generationMethod === 'phase_based' && (
                  <>
                    <Button
                      ...
                      className="rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold px-6 py-3 text-lg shadow-lg hover:shadow-xl transition-all duration-200"
                      size="lg"
                    >
                      {isGeneratingPhaseBased ? (
                        <><LoadingInline /> <span className="ml-2">段階的配置生成中...</span></>
                      ) : (
                        <><Sparkles className="w-5 h-5 mr-2" /> 段階的配置で自動生成</>
                      )}
                    </Button>
                    {assignments.length > 0 && (
                      <Button
            ...
            <Button variant="outline" onClick={handleExportPDF} className="rounded-xl bg-white hover:bg-gray-50">
              <FileDown className="w-4 h-4 mr-2" />
              PDF出力
            </Button>
            {currentShift.status !== "ai_generated" && (
              <Button variant="outline" onClick={handleSave} className="rounded-xl bg-white hover:bg-gray-50">
        ...
        {viewMode === "table-v2" && (
          <ShiftTableV2
            ...
            employees={employees.map(emp => ({
              id: emp.dbId || 0,  // 数値データベースIDを使用
              employeeId: emp.id || '',  // 表示用ID（例: "0034"）
              name: emp.name,
