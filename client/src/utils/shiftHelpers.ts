...
// 読み取り専用判定（confirmed, actual, archivedは編集不可）
export function isReadOnlyStatus(status: ShiftStatus): boolean {
  return ["ai_generated", "confirmed", "actual", "archived"].includes(status);
}
